  ...
  fetch('/artworks_IDs.json')
    ...
    .then((data) => {
      ...
      const randomNumber = Math.floor(
        Math.random() * (Math.floor(ID.length) - Math.ceil(1) + 1) +
          Math.ceil(1)
      );
...
class Postcard {
  ...
  displayImage() {
    if (this.thumbnail === null || this.thumbnail === undefined) {
      document.querySelector('#postcard-image').src =
        'https://images.unsplash.com/photo-1651098527823-d24f680f3f09?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=MnwxfDB8MXxyYW5kb218MHx8fHx8fHx8MTY1MTUxMzQ2MA&ixlib=rb-1.2.1&q=80&w=1080';
    } else {
  ...
  displayArtworkInfo() {
    document.querySelector(
      '#artworkInfo'
    ).innerHTML = `${this.artist}: ${this.title}, ${this.date},<br>${this.medium_display}`;
  }

  setImageAlt() {
    if (
      this.thumbnail['alt_text'] === null ||
      this.thumbnail['alt_text'] === undefined
    ) {
      document.querySelector('#postcard-image').alt =
        'No Alt Description for this image';
    } else {
      document.querySelector(
        '#postcard-image'
      ).alt = `${this.thumbnail['alt_text']}`;
    }
  }
}
